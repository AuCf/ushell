      ...
      <div className={`h-9 px-2.5 border-b flex items-center justify-between font-mono gap-1 shrink-0 ${
        ...
        <div className="flex items-center gap-1.5 font-bold tracking-tight min-w-0 flex-1 overflow-hidden">
          ...
          <button
            ...
            title={hasUpdate ? (lang === 'zh' ? '检测到新版本可用！点击进行热更新升级' : 'New version available! Click to update') : `uShell v${packageJson.version}`}
          >
            <span className={`text-[9px] font-normal shrink-0 ${isLight ? 'text-slate-500' : 'text-zinc-500'}`}>v{packageJson.version}</span>
            {hasUpdate && (
