...
export const TabBar: React.FC<TabBarProps> = ({
  ...
  const handleWindowMaximize = (e: React.MouseEvent) => {
    e.stopPropagation();
    try {
  ...
  const handleWindowClose = (e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      getCurrentWindow().close();
    } catch (err) {
      try {
        (window as any).__TAURI__?.window?.getCurrentWindow()?.close() || window.close();
      } catch (e) {
        window.close();
      }
    }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-0.5 overflow-x-auto no-scrollbar flex-1 h-full" data-tauri-drag-region>
        ...
        {/* Empty drag space between tabs and right controls */}
        <div className="flex-1 h-full" data-tauri-drag-region />
      </div>

      {/* View Switcher, Language Toggle, Theme Toggle & Non-Drag Window Controls */}
      <div className="flex items-center gap-2 z-20 pointer-events-auto" data-tauri-drag-region={false}>
        {activeTab && (
        ...
        {/* Window Controls (Explicitly exempted from drag region) */}
        <div 
